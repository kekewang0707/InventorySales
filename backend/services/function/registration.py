...
tools.register_tool(Tool(
    "list_delivery_notes", "查询送货单列表，可按客户、日期范围筛选", {
        "type": "object", "properties": {
...
tools.register_tool(Tool(
    "get_delivery_note", "查看送货单详情，包含明细行和产品信息", {
        "type": "object", "properties": {
...
tools.register_tool(Tool(
    "get_statement", "查询客户对账单，支持指定客户或全部客户", {
        "type": "object", "properties": {
...
tools.register_tool(Tool(
    "get_statistics", "查看系统统计概览：产品总数、客户总数、送货单总数、本月金额", {
        "type": "object", "properties": {},
        "additionalProperties": False,
    },
...
tools.register_tool(Tool(
    "create_delivery_note", "创建送货单（需要用户二次确认后才执行）", {
        "type": "object", "properties": {
            "customer_id": {"type": "integer", "description": "客户ID"},
    ...
    security="write",
    write_handler=handlers.handle_create_delivery_note,
...
tools.register_tool(Tool(
    ...
    security="write",
    write_handler=handlers.handle_advance_note_status,
    summarize_handler=handlers.summarize_advance_note_status,
))

# ---- 导出供外部使用 ----

FUNCTION_DEFINITIONS = tools.openai_definitions
_QUERY_TOOLS = tools.query_tools
